    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          ...
          size={13}
          style={{
            color: i <= rating ? "var(--warning)" : "var(--surface-3)",
  ...
  if (!seller) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4" style={{ background: "var(--bg)" }}>
        <Navbar />
        <p style={{ color: "var(--text-secondary)" }}>Vendedor não encontrado</p>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {seller.avgRating !== null ? (
                  <div className="flex items-center gap-1.5">
                    <Stars rating={Math.round(seller.avgRating)} />
                    <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
                      {seller.avgRating.toFixed(1)}
                    </span>
                    <span className="text-xs" style={{ color: "var(--text-secondary)" }}>
                      ({seller.reviews.length} avaliação{seller.reviews.length !== 1 ? "ões" : ""})
                    </span>
                  </div>
                ) : (
                  <span className="text-xs" style={{ color: "var(--text-secondary)" }}>
                ...
                <div className="flex items-center gap-1 text-xs" style={{ color: "var(--text-secondary)" }}>
                  <Package size={12} />
                  {seller.listings} anúncio{seller.listings !== 1 ? "s" : ""} realizados
                </div>
                <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>·</span>
                <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>
                  Membro desde {new Date(seller.createdAt).toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}
                </span>
              </div>
            </div>
          </div>
